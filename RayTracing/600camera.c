...
void camWorldFromScreenHomogeneous(const camCamera *cam, double width,
		double height, double homog[4][4]) {
		 	/* Screen coords to world coords -> (C * P^-1 * V^-1). */

			/* This gets C for perspective. */
			double camHomog[4][4];
			isoGetHomogeneous(&cam->isometry, camHomog);

			/* This gets P^-1 for perspective. */
			double projInv[4][4];
			camGetInversePerspective(cam, projInv);

			/* This gets V^-1 for viewport. */
			double viewportInv[4][4];
			mat44InverseViewport(width, height, viewportInv);

			/* Multiply into homog. */
			double viewingInv[4][4];
			mat444Multiply(camHomog, projInv, viewingInv);
			mat444Multiply(viewingInv, viewportInv, homog);
}
