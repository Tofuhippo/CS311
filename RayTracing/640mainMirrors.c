...
/* Light stuff. */
// Diffuse
double dFromLight[3] = {10.0, 10.0, 1.0}; // World coords
...
void sphereColor(const isoIsometry *isom, double radius, const double e[3],
		const double d[3], double tEnd, const texTexture *tex, double rgb[3]) {
			...
			double dLight[3];
			vecScale(3, -1, dFromLight, dLight); // Get direction TOWARDS the light, World
			...
			/* Shadowing. */
			int shadowed = 0; // True = 1/False = 0
			double tStartShadow = EPSILON;
			double tEndShadow = INFINITY;
			rayRecord shadowRayA, shadowRayB;
			...
			if (iDiff > 0) {
				...
				d is now dtoLight. Test for both body A and B. */
					/* Check isomA. */
				shadowRayA = sphereIntersection(&isomA, radiusA, xWorld,
																			  dLight, tStartShadow, tEndShadow);
				/* Check isomB. */
				shadowRayB = sphereIntersection(&isomB, radiusB, xWorld,
																			  dLight, tStartShadow, tEndShadow);
