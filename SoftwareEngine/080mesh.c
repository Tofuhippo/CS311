...
void meshRender(const meshMesh *mesh, const shaShading *sha,
		const double unif[], const texTexture *tex[]) {
			...
			for (int i = 0; i < mesh->triNum; i++){
				...
				transformVertex(sha->unifDim, unif, sha->attrDim, tempVertexA,
												sha->varyDim, varyA);
				transformVertex(sha->unifDim, unif, sha->attrDim, tempVertexB,
					              sha->varyDim, varyB);
				transformVertex(sha->unifDim, unif, sha->attrDim, tempVertexC,
					              sha->varyDim, varyC);
