...
/* Performs homogeneous division, viewport transformation, and triangle rendering */
void homogDivViewportRender(depthBuffer *buf, const double viewport[4][4],
		const shaShading *sha, const double unif[], const texTexture *tex[],
		double vary0[], double vary1[], double vary2[]) {

	int varyDim = sha->varyDim;

	// Viewport transformation
	double screenVary0[varyDim];
	double screenVary1[varyDim];
	double screenVary2[varyDim];
	vecCopy(varyDim, vary0, screenVary0);
	vecCopy(varyDim, vary1, screenVary1);
	vecCopy(varyDim, vary2, screenVary2);
	mat441Multiply(viewport, vary0, screenVary0);
	mat441Multiply(viewport, vary1, screenVary1);
	mat441Multiply(viewport, vary2, screenVary2);


	// Homogeneous division
	double wInv0 = 1.0 / screenVary0[3];
	double wInv1 = 1.0 / screenVary1[3];
	double wInv2 = 1.0 / screenVary2[3];
	vecScale(varyDim, wInv0, screenVary0, screenVary0);
	vecScale(varyDim, wInv1, screenVary1, screenVary1);
	vecScale(varyDim, wInv2, screenVary2, screenVary2);
	...
	triRender(sha, buf, unif, tex, screenVary0, screenVary1, screenVary2);
}

/* Returns 1 if vertex should be clipped, 0 otherwise */
int detectClip(double vary[]) {
	if (vary[3] <= 0.0)
		return 1;
	else {
		if (vary[3] < -vary[2])
...
/* Linearly interpolates the clipped vertex a (of dimension dim) with the
non-clipped vertex b and stores the result in c */
void clippingInterpolation(int dim, double a[], double b[], double c[]) {
	double t = (a[2] + a[3]) / (a[2] + a[3] - b[2] - b[3]);
	vecSubtract(dim, b, a, c);
	vecScale(dim, t, c, c);
	vecAdd(dim, a, c, c);
}
...
void meshRender(const meshMesh *mesh, depthBuffer *buf,
	...
	if (mesh->attrDim != sha->attrDim) {
		printf("error: mesh and shading have different values for attrDim\n");
		return;
	}

	for (int tri = 0; tri < mesh->triNum; tri += 1) {
		// Get vertices
		int *triangle = meshGetTrianglePointer(mesh, tri);
		double *v0 = meshGetVertexPointer(mesh, triangle[0]);
		double *v1 = meshGetVertexPointer(mesh, triangle[1]);
		double *v2 = meshGetVertexPointer(mesh, triangle[2]);

		// Initialize varyings
		int varyDim = sha->varyDim;
		double vary0[varyDim];
		double vary1[varyDim];
		double vary2[varyDim];

		// Transform vertices
		sha->transformVertex(sha->unifDim, unif, sha->attrDim, v0, varyDim, vary0);
		sha->transformVertex(sha->unifDim, unif, sha->attrDim, v1, varyDim, vary1);
		sha->transformVertex(sha->unifDim, unif, sha->attrDim, v2, varyDim, vary2);

		// Find vertices to clip
		int clip0 = detectClip(vary0);
		int clip1 = detectClip(vary1);
		int clip2 = detectClip(vary2);
		int numClip = clip0 + clip1 + clip2;

		// Skip triangle if all three vertices clipped
		if (numClip == 3)
			continue;

		// Draw smaller triangle if two vertices clipped
		else if (numClip == 2) {
			double clippedA[varyDim]; double clippedB[varyDim];
			// Clip vertices 0 and 1
			if (!clip2) {
				clippingInterpolation(varyDim, vary0, vary2, clippedA);
				clippingInterpolation(varyDim, vary1, vary2, clippedB);
				homogDivViewportRender(buf, viewport, sha, unif, tex, clippedA, clippedB, vary2);
			}
			// Clip vertices 0 and 2
			else if (!clip1) {
				clippingInterpolation(varyDim, vary0, vary1, clippedA);
				clippingInterpolation(varyDim, vary2, vary1, clippedB);
				homogDivViewportRender(buf, viewport, sha, unif, tex, clippedA, vary1, clippedB);
			}
			// Clip vertices 1 and 2
			else
				clippingInterpolation(varyDim, vary1, vary0, clippedA);
				clippingInterpolation(varyDim, vary2, vary0, clippedB);
				homogDivViewportRender(buf, viewport, sha, unif, tex, vary0, clippedA, clippedB);
		}

		// Draw 2 triangles if two vertices clipped
		else if (numClip == 1) {
			double clippedA[varyDim]; double clippedB[varyDim];
			// Clip vertex 0
			if (clip0) {
				clippingInterpolation(varyDim, vary0, vary1, clippedA);
				clippingInterpolation(varyDim, vary0, vary2, clippedB);
				homogDivViewportRender(buf, viewport, sha, unif, tex, clippedB, clippedA, vary1);
				homogDivViewportRender(buf, viewport, sha, unif, tex, clippedB, vary1, vary2);
			}
			// Clip vertex 1
			else if (clip1) {
				clippingInterpolation(varyDim, vary1, vary0, clippedA);
				clippingInterpolation(varyDim, vary1, vary2, clippedB);
				homogDivViewportRender(buf, viewport, sha, unif, tex, clippedB, clippedA, vary0);
				homogDivViewportRender(buf, viewport, sha, unif, tex, clippedB, vary0, vary2);
			}
			// Clip vertex 2
			else {
				clippingInterpolation(varyDim, vary2, vary0, clippedA);
				clippingInterpolation(varyDim, vary2, vary1, clippedB);
				homogDivViewportRender(buf, viewport, sha, unif, tex, clippedB, clippedA, vary0);
				homogDivViewportRender(buf, viewport, sha, unif, tex, clippedB, vary0, vary1);
			}
		}

		// Draw triangle normally if no vertices clipped
		else
			homogDivViewportRender(buf, viewport, sha, unif, tex, vary0, vary1, vary2);
	}
}
